...
DownloadUpdateArtifacts() {
    ...
    # Notify that new firmware is comming
    # Show output.
    output "Result:" "$aduc_result"
...
InstallUpdate() {
    ...
    file_path="/var/lib/adu/aduUserConfirm"
    while true; do
        # Read the content of the file
        content=$(cat "$file_path")

        # Check if the content is equal to 2
        if [[ "$content" == "2" ]]; then
            echo "Content is 2. Exiting loop."
            break
        else
            echo "Content is not 2. Waiting for the content to be 2..."
            # You can add a sleep to avoid continuous checking and reduce system load
...
ApplyUpdate() {
    ...
    file_path="/var/lib/adu/aduNewFw"

    # Write content to the file
    echo "$content" > "$file_path"
    file_path_user_confirm="/var/lib/adu/aduUserConfirm"
    while true; do
        # Read the content of the file
        content=$(cat "$file_path_user_confirm")

        # Check if the content is equal to 2
        if [[ "$content" == "4" ]]; then
            echo "Content is 4. Exiting loop."
            break
        else
            echo "Content is not 4. Waiting for the content to be 2..."
            # You can add a sleep to avoid continuous checking and reduce system load
            sleep 5  # Adjust the sleep duration as needed
        fi
    done
    ...
    echo "FW_SETENV:"
    echo ${update_partition}
    log_info "FW_SETENV: \"$update_partition\""
...
# Validate current fw with the new fw
ValidateUpdate(){
    log_info "Validating update..."
    # Read /etc/adu-version file to get the current version
    current_version=$(cat /etc/adu-version)
    # Read /var/lib/adu/aduNewFw file to get the new version
    new_version=$(cat /var/lib/adu/aduNewFw)
    ...
    # Sample foe current_version: 2.1, Sample for new_version: 2.2
    # Check if the new version is greater than the current version
    if [[ "$new_version" > "$current_version" ]]; then
        # If the new version is greater than the current version, return 0
        ret_val=0

    else
        # If the new version is not greater than the current version, return 1
        ret_val=1
    fi
    # Return the result
    $ret $ret_val
